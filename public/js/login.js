document.addEventListener('DOMContentLoaded', async () => {
    const apiFetch = window.KpvsApi && window.KpvsApi.apiFetch
        ? window.KpvsApi.apiFetch.bind(window)
        : function (url, init) {
    ...
    form.addEventListener('submit', async e => {
        ...
        try {
            ...
            if (isRecover) {
                const res = await apiFetch('/api/user/auth/recover', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
            ...
            if (resetToken) {
                const res = await apiFetch('/api/user/auth/reset', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
