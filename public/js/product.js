...
async function loadProduct() {
    ...
    try {
        ...
        productMainEl.innerHTML =
            ...
            '<button type="button" class="btn btn--primary product-page-action-btn favorite-action-btn ' +
            (isFavorite ? 'in-favorites' : '') +
            '" data-action="product-favorite">' +
            (isFavorite ? 'Удалить из избранного' : 'Добавить в избранное') +
            '</button>' +
            '<button type="button" class="btn btn--primary product-page-action-btn cart-action-btn ' +
            (isInCart ? 'in-cart' : '') +
            '" data-action="product-cart">' +
            (isInCart ? 'Удалить из корзины' : 'В корзину') +
            '</button>' +
            '<button type="button" class="btn btn--secondary inquire-action-btn" data-action="product-inquire">Запросить цену</button>' +
            '</div>' +
            '</div>' +
        ...
        const imageBlock = productMainEl.querySelector('.product-image-block');
        if (imageBlock && images.length > 1) {
            const galleryEl = document.createElement('div');
            galleryEl.className = 'product-gallery';
            images.forEach(function (img) {
                const u = sanitizeProductImageUrl(img.url || '');
                if (!u) return;
                const thumb = document.createElement('img');
                thumb.className = 'product-gallery-item';
                thumb.dataset.src = u;
                thumb.alt = product.name == null ? '' : String(product.name);
                thumb.src = u;
                galleryEl.appendChild(thumb);
            });
            const actions = imageBlock.querySelector('.product-actions');
            if (actions) {
                imageBlock.insertBefore(galleryEl, actions);
            } else {
                imageBlock.appendChild(galleryEl);
            }
        }
