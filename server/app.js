...
function applyCsrfWhenNeeded(req, res, next) {
...
function readUserJwtToken(req) {
    const fromCookie = req.cookies && req.cookies[JWT_COOKIE_NAME] ? String(req.cookies[JWT_COOKIE_NAME]).trim() : '';
    if (fromCookie) return fromCookie;
    return getBearerToken(req);
}

function requireUserJwt(req, res, next) {
    const token = readUserJwtToken(req);
...
function renderErrorHtml(statusCode) {
    ...
    const errorPath = path.join(PUB_ROOT, 'error.html');
...
app.get('/api/csrf-token', (req, res, next) => {
    csrfProtection(req, res, (err) => {
        ...
        res.type('application/json').json({ csrfToken: req.csrfToken() });
    });
});
...
app.use((req, res, next) => {
    ...
    if (!dbHealthy && typeof req.path === 'string' && req.path.startsWith('/api')) {
...
app.post('/api/user/auth/login', async (req, res) => {
    try {
        ...
        res.cookie(JWT_COOKIE_NAME, token, userJwtCookieOptions());
...
app.patch('/api/user/auth/password', requireUserJwt, async (req, res) => {
    try {
        ...
        if (token) res.cookie(JWT_COOKIE_NAME, token, userJwtCookieOptions());
...
async function bootDatabase() {
    ...
    await ensureProductsEditorColumn();
    await ensureCollectionsSchema();
    await ensureCategorySizeTypesSchema();
    await ensureSizeGroupsSchema();
    await ensureSizesUniqueValueIndex();
    await ensureReferenceSizesSeed();
    dbHealthy = true;
}
