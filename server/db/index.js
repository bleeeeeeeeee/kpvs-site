...
const { pool, isProduction } = require('./pool');
const { publicMediaUrl, mapProductRowMedia, mapBrandRowMedia } = require('./media-url');
...
async function createProduct(data, ctx = {}) {
    ...
    try {
        ...
        await client.query('COMMIT');
        return getProduct(String(productId), { includeInactive: true });
    } catch (err) {
        try { await client.query('ROLLBACK'); } catch {}
        throw err;
    } finally {
...
async function updateProduct(id, data, ctx = {}) {
    ...
    try {
        ...
        await client.query('COMMIT');
        return getProduct(String(id), { includeInactive: true });
    } catch (err) {
        try { await client.query('ROLLBACK'); } catch {}
        throw err;
    } finally {
