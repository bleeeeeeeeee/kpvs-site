...
async function loadProducts() {
    try {
        const res = await fetch('/api/products/womens');
...
function toggleCart(productId, buttonElement) {
    ...
    if (existingIndex === -1) {
        // Добавляем в корзину
        cart.push({ id: productId, source: 'womens' });
...
function toggleFavorite(productId, buttonElement = null) {
    ...
    if (wasFavorite) {
        ...
        favorites.push({ id: productId, source: 'womens' });
    }
                ...
                <div class="modal-body">
                    <div class="modal-items">
                        ${products.map((product, idx) => {
                            ...
                            const source = favorites[idx].source === 'womens' ? '(женское)' : (favorites[idx].source === 'mens' ? '(мужское)' : '(товар)');
                            return `
